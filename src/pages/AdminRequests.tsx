...
import { useAdminRequests, useApproveRequest, useRejectRequest } from "@/hooks/useAdminData";
import { 
  Dialog, 
  DialogContent, 
  DialogHeader, 
...
export default function AdminRequests() {
    ...
    try {
      if (actionType === "approve") {
        await approveMutation.mutateAsync({
          ...
          days: selectedRequest.type === 'deletion' && deletionType === "scheduled" ? deletionDays : undefined,
          isManual: selectedRequest.type === 'deletion' && deletionType === "immediate"
        });
        toast({ 
          title: t("common.success"), 
          description: selectedRequest.type === 'deletion' && deletionType === "scheduled" ? t("deletion.scheduled") : t("common.success") 
        });
      } else {
  ...
  const filteredRequests = (type: string) => requests?.filter((r: any) => {
    ...
    >
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 relative z-10">
        <div className="flex flex-col md:flex-row md:items-center gap-8 flex-1">
          ...
          <div className="flex items-center gap-5">
            <div className="h-14 w-14 rounded-[20px] bg-primary/10 text-primary flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform">
              {request.type === 'deletion' ? <UserX className="h-7 w-7" /> : <FileDown className="h-7 w-7" />}
            </div>
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                {request.type === 'backup' && (
                   <span className="text-[10px] font-black bg-amber-500/10 text-amber-600 px-2.5 py-1 rounded-lg uppercase tracking-wider border border-amber-500/20">
                     {request.backup_format?.toUpperCase()}
                   </span>
                )}
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 md:flex items-center gap-6">
            ...
            <div className="flex items-center gap-2.5 border-l border-border/50 pl-6">
              ...
              <span className={cn(
                ...
                request.status === 'approved' ? "text-emerald-600" : "text-red-700"
              )}>
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <Button
              onClick={() => { 
                ...
                // Set default deletion type to immediate when approving
        ...
        {request.admin_message && (
          <div className="mt-4 p-4 rounded-2xl bg-muted/40 border border-border/50 flex items-start gap-3 w-full lg:w-auto">
      ...
      <div className="space-y-8 animate-in fade-in duration-700 pb-20">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-card/40 backdrop-blur-md p-8 rounded-[40px] border border-border/50 shadow-sm relative overflow-hidden">
          <div className="flex items-center gap-5 relative z-10">
            ...
            <div>
              <h1 className="text-3xl font-black tracking-tight text-foreground uppercase">{t("nav.adminRequests")}</h1>
              <p className="text-xs font-bold text-muted-foreground uppercase tracking-[0.2em] opacity-60 mt-1">Unified Management Center</p>
            </div>
          </div>
        ...
        {/* Requests Tabs */}
        <Tabs defaultValue="deletion" className="space-y-6">
          <TabsList className="bg-card/40 backdrop-blur-md p-1.5 rounded-[24px] h-16 border border-border/50 w-full md:w-80 grid grid-cols-2">
            <TabsTrigger value="deletion" className="rounded-2xl font-black uppercase text-[10px] tracking-widest data-[state=active]:bg-primary data-[state=active]:text-primary-foreground transition-all">
              {t("requests.deletionTab")}
              {filteredRequests('deletion')?.filter(r => r.status === 'pending').length > 0 && (
                <span className="ml-2 h-5 w-5 bg-red-500 text-white rounded-full flex items-center justify-center text-[10px] animate-pulse">
                  {filteredRequests('deletion')?.filter(r => r.status === 'pending').length}
                </span>
              )}
            </TabsTrigger>
            <TabsTrigger value="backup" className="rounded-2xl font-black uppercase text-[10px] tracking-widest data-[state=active]:bg-primary data-[state=active]:text-primary-foreground transition-all">
              {t("requests.backupTab")}
              {filteredRequests('backup')?.filter(r => r.status === 'pending').length > 0 && (
                <span className="ml-2 h-5 w-5 bg-amber-500 text-white rounded-full flex items-center justify-center text-[10px] animate-pulse">
                  {filteredRequests('backup')?.filter(r => r.status === 'pending').length}
                </span>
              )}
            </TabsTrigger>
          </TabsList>

          <TabsContent value="deletion" className="space-y-4 focus-visible:outline-none">
            {isLoading ? (
              <SkeletonLoader rows={3} />
            ) : filteredRequests('deletion')?.length === 0 ? (
              ...
              filteredRequests('deletion')?.map(renderRequestCard)
            )}
          </TabsContent>
          ...
          <TabsContent value="backup" className="space-y-4 focus-visible:outline-none">
             {isLoading ? (
              <SkeletonLoader rows={3} />
            ) : filteredRequests('backup')?.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-20 bg-card/40 backdrop-blur-sm rounded-[40px] border border-dashed border-border/50">
                <AlertCircle className="h-10 w-10 text-muted-foreground mb-4" />
                <p className="text-lg font-black text-muted-foreground/60 uppercase tracking-widest">{t("common.noData")}</p>
              </div>
            ) : (
              filteredRequests('backup')?.map(renderRequestCard)
            )}
          </TabsContent>
        </Tabs>
          ...
          <DialogContent className="rounded-[40px] border-none shadow-2xl backdrop-blur-xl bg-white/90 p-8 max-w-md">
            <DialogHeader className="space-y-4">
              <div className={cn(
                ...
                {actionType === "approve" ? (selectedRequest?.type === 'deletion' ? <Trash2 className="h-8 w-8" /> : <FileDown className="h-8 w-8" />) : <XCircle className="h-8 w-8" />}
              </div>
              ...
              <DialogDescription className="text-center font-bold text-xs text-muted-foreground leading-relaxed px-4">
                {actionType === "approve" 
                  ? (selectedRequest?.type === 'deletion' ? t("deletion.approveConfirm") : t("requests.approveBackup")) 
                  : t("deletion.rejectConfirm")}
              </DialogDescription>
            </DialogHeader>

            <div className="mt-8 space-y-4">
               {actionType === "approve" && selectedRequest?.type === 'deletion' && (
                 <>
